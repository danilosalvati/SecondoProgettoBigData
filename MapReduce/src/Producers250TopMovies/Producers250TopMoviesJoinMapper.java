...
/**
 * Il mapper costruisce le coppie (film, produttore) o (film, "<TOJOIN>") a
 * seconda se sta parsando il file dei produttori o quello dei 250 top film e le
 * invia al reducer che fa il join
 *
 */
		...
		if (value.toString().startsWith("0000")) {
			...
			if (values[1].toString().contains("<ENDVALUE>")) {
				...
				for (String movie : movies) {
					/*
					 * Scrivo la coppia (film, produttore) per tutti i film del
					 * produttore
					 */
					title = movie.split("\\)")[0] + ")";

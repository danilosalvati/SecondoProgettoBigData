...
/**
 * Il mapper costruisce le coppie (keyword, 1) se il film è nei top 250 e le
 * invia al reducer
 *
 */
public class Keywords250TopMoviesMapper extends
		Mapper<Object, Text, Text, IntWritable> {

	/*
	 * Il Set è necessario per evitare di scrivere più volte la stesso film per
	 * la stessa keyword (dovuto a matching multipli della stessa sottostringa)
	 */
	private static Set<String> keywordMoviePairs = new HashSet<String>();
	...
	public void map(Object key, Text value, Context context)
		...
		for (int i = 0; i < listSize && !found; i++) {
			if (values[0].equals(top250movies.get(i))
					&& keywordMoviePairs.add(top250movies.get(i) + values[1])) {
